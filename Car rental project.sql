...
alter table payment add foreign key(lease_id) references lease(lease_id) on delete set null;
alter table lease add foreign key(vehicle_id) references vehicle(vehicle_id) on delete set null;
alter table lease add foreign key(customer_id) references customer(customer_id) on delete set null;
...
insert into lease values(10,10,10,'2023-10-10','2023-10-31','Monthly');
insert into lease values(14,1,1,'2024-03-01','2024-03-31','Monthly');
...
insert into payment values(13,14,'2024-03-05',1500.00);
...
SELECT l.lease_id, l.lease_startdate, l.lease_enddate
FROM lease l
JOIN payment p ON l.lease_id = p.lease_id
WHERE l.lease_startdate <= CURDATE()
AND l.lease_enddate >= CURDATE()
AND p.transactionDate > DATE_SUB(CURDATE(), INTERVAL 1 MONTH)
GROUP BY l.lease_id
